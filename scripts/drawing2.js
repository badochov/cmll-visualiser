let tile=75,
	edges=[[1.5,0],[1.5,1.5],[0.5,2.5],[2.5,2.5],[1.5,3.5],[1.5,5]],
	corners=[[[2.5,3.5],[3.5,3.5],[2.5,4.5]],
			 [[2.5,1.5],[3.5,1.5],[2.5,1.25]],
			 [[0.5,1.5],[0.25,1.5],[0.5,1.25]],
			 [[0.5,3.5],[0.25,3.5],[0.5,4.5]]],
	cor=[[[0,1,2],0],
		 [[0,2,3],0],
		 [[0,3,4],0],
		 [[0,4,1],0]],
	cp=[0,1,2,3],
	colors=[$('#top').spectrum('get').toHexString(),
...
function fillTile(n){
	ctx.fillStyle=color;
	ctx.fillRect(edges[n][0]*tile+lineWidth/2,edges[n][1]*tile+lineWidth,tile-lineWidth,tile-lineWidth);
}
function fillCorner(n){
	...
	ctx.fillRect(corners[n][0][0]*tile+lineWidth/2,corners[n][0][1]*tile+lineWidth,tile-lineWidth,tile-lineWidth);
	
	ctx.fillStyle=colors[cor[cp[n]][0][(a+cor[cp[n]][1])%3]];
	ctx.fillRect(corners[n][1][0]*tile+lineWidth/2,corners[n][1][1]*tile+lineWidth,tile*0.25-lineWidth,tile-lineWidth);
	
	ctx.fillStyle=colors[cor[cp[n]][0][(b+cor[cp[n]][1])%3]];
	ctx.fillRect(corners[n][2][0]*tile+lineWidth/2,corners[n][2][1]*tile+lineWidth,tile-lineWidth,tile*0.25-lineWidth);
}
function drawCorner(n){
	drawTile(corners[n][0][0],corners[n][0][1]);
	ctx.strokeRect(corners[n][1][0]*tile,corners[n][1][1]*tile+lineWidth/2,tile*0.25,tile);
	ctx.strokeRect(corners[n][2][0]*tile,corners[n][2][1]*tile+lineWidth/2,tile,tile*0.25);
}
function drawCross(){
	ctx.clearRect(0,0, 4*tile,6*tile+lineWidth);	
	ctx.fillStyle=bgColor;
