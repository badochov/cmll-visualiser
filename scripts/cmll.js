...
$(document).ready(() => {
  ...
  $("body").keydown((e) => {
    if ($(e.target).is("textarea") || $(e.target).is("input")) return;
    e.preventDefault();
    switch (e.keyCode) {
      case 39:
        next();
...
function generateState() {
  if (scrambles.length == 0) return false;
...
function drawState(lse) {
  ...
  if (drawEP) {
    for (let i = 0; i < 6; i++) {
      if (lse[i][0] != i) drawArrow(lse[i][0], i);
    }
  }
}
function decodeState() {
  let lse = [];
  for (let i = 0; i < 6; i++) lse[i] = [];
...
function decodeScramble(scramble) {
  ...
  for (let i = 0; i < moves.length; i++) {
    ...
    let num = 1;
    if (moves[i].length > 1) {
      if (!isNaN(parseInt(moves[i][1]))) num = parseInt(moves[i][1]) % 4;
      if (isNaN(parseInt(moves[i].substr(-1)))) num = 4 - num;
    }
    if (premove) {
      num = 4 - num;
      premove = false;
...
function ifCMLL() {
  ...
  for (let i = 0; i < corn.length; i++) {
    if (i > 3 && corn[i][0] != i && corn[i][1] % 3 != 0) return false;
  }
  return true;
}
...
function getScrambles() {
  scrambles = $("#scrambles")
    .val()
    .replace(new RegExp(String.fromCharCode(parseInt(2019, 16)), "g"), "'")
    .split("\n")
    .map((s) => s.trim())
    .filter((value) => {
      return value != "";
...
function createPDF() {
  name = $("#filename").val();
  name =
    name == "" || name == undefined ? $("#filename").attr("placeholder") : name;
  ...
  for (let i = 0; i < scrambles.length; i++) {
    ...
    centeredText(
      scrambles[i].includes("//") ? scrambles[i].split("//")[1].trim() : "",
      y + 15,
      doc,
      i % 2
    );
...
var centeredText = function (text, y, doc, even) {
  ...
  var textOffset =
    (doc.internal.pageSize.getWidth() / 2 - textWidth) / 2 +
    (doc.internal.pageSize.getWidth() / 2) * even;
